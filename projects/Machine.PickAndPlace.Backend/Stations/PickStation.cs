using NAutoSuite.Core.Abstractions;
using NAutoSuite.Core.Common;
using Serilog;

namespace PickAndPlace.Backend.Stations;

/// <summary>
/// Station responsible for picking parts
/// </summary>
public class PickStation
{
    private readonly IAxis? _axisX;
    private readonly IAxis? _axisY;
    private readonly IAxis? _axisZ;
    private readonly IOutput? _vacuum;
    private readonly ILogger _logger;

    // Pick position coordinates
    private const double PickX = 100.0;
    private const double PickY = 50.0;
    private const double PickZDown = 10.0;
    private const double PickZUp = 50.0;

    public PickStation(
        IAxis? axisX,
        IAxis? axisY,
        IAxis? axisZ,
        IOutput? vacuum,
        ILogger logger)
    {
        _axisX = axisX;
        _axisY = axisY;
        _axisZ = axisZ;
        _vacuum = vacuum;
        _logger = logger;
    }

    public async Task<Result> ExecuteAsync(CancellationToken cancellationToken = default)
    {
        _logger.Information("Pick sequence started");

        try
        {
            // Move to pick position
            var result = await MoveToPickPositionAsync(cancellationToken);
            if (!result.IsSuccess)
            {
                _logger.Error("Failed to move to pick position: {Message}", result.Message);
                return result;
            }

            // Lower Z
            result = await LowerZAsync(cancellationToken);
            if (!result.IsSuccess)
            {
                _logger.Error("Failed to lower Z: {Message}", result.Message);
                return result;
            }

            // Activate vacuum
            result = await ActivateVacuumAsync(cancellationToken);
            if (!result.IsSuccess)
            {
                _logger.Error("Failed to activate vacuum: {Message}", result.Message);
                return result;
            }

            // Raise Z
            result = await RaiseZAsync(cancellationToken);
            if (!result.IsSuccess)
            {
                _logger.Error("Failed to raise Z: {Message}", result.Message);
                return result;
            }

            _logger.Information("Pick sequence completed successfully");
            return Result.Success("Pick completed");
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Pick sequence failed");
            return Result.Failure("Pick sequence failed", ex);
        }
    }

    private async Task<Result> MoveToPickPositionAsync(CancellationToken cancellationToken)
    {
        if (_axisX != null)
        {
            var result = await _axisX.MoveAbsoluteAsync(PickX, cancellationToken: cancellationToken);
            if (!result.IsSuccess)
                return Result.Failure($"Axis X move failed: {result.Message}");
        }

        if (_axisY != null)
        {
            var result = await _axisY.MoveAbsoluteAsync(PickY, cancellationToken: cancellationToken);
            if (!result.IsSuccess)
                return Result.Failure($"Axis Y move failed: {result.Message}");
        }

        // Wait for axes to settle
        await Task.Delay(500, cancellationToken);

        return Result.Success("Moved to pick position");
    }

    private async Task<Result> LowerZAsync(CancellationToken cancellationToken)
    {
        if (_axisZ != null)
        {
            var result = await _axisZ.MoveAbsoluteAsync(PickZDown, cancellationToken: cancellationToken);
            if (!result.IsSuccess)
                return Result.Failure($"Z axis down failed: {result.Message}");

            await Task.Delay(300, cancellationToken);
        }

        return Result.Success("Z lowered");
    }

    private async Task<Result> RaiseZAsync(CancellationToken cancellationToken)
    {
        if (_axisZ != null)
        {
            var result = await _axisZ.MoveAbsoluteAsync(PickZUp, cancellationToken: cancellationToken);
            if (!result.IsSuccess)
                return Result.Failure($"Z axis up failed: {result.Message}");

            await Task.Delay(300, cancellationToken);
        }

        return Result.Success("Z raised");
    }

    private async Task<Result> ActivateVacuumAsync(CancellationToken cancellationToken)
    {
        if (_vacuum != null)
        {
            var result = await _vacuum.WriteAsync(true, cancellationToken);
            if (!result.IsSuccess)
                return Result.Failure($"Vacuum activation failed: {result.Message}");

            await Task.Delay(200, cancellationToken);
        }

        return Result.Success("Vacuum activated");
    }
}
