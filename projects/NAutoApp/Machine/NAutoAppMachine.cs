...
namespace NAutoApp.Machine;

/// <summary>
/// NAutoApp sample with 3 axes and simulated EtherCAT IO.
/// </summary>
public class NAutoAppMachine : MachineBase
{
    ...
    private Cylinder _rejectCylinder;
    private readonly SensorWaiter _sensorWaiter;
    private string _partPresentAddress = string.Empty;
    private string _testOkAddress = string.Empty;
    private string _testNgAddress = string.Empty;
    private string _cylinderExtendedAddress = string.Empty;
    private string _cylinderRetractedAddress = string.Empty;
    private string _cylinderExtendAddress = string.Empty;
    private string _vacuumOkAddress = string.Empty;
    private string _vacuumOnAddress = string.Empty;
    ...
    public NAutoAppMachine(
        ...
        ILogger? logger = null)
        : base("PAP_SIM", "NAutoApp Simulator", profile ?? new ConfigMachineProfile(), logger)
    {
        ...
        RefreshEntityAddresses();
        _rejectCylinder = new Cylinder(IO, _cylinderExtendAddress, _cylinderExtendedAddress, _cylinderRetractedAddress, logger);
                ...
                switch (step)
                {
                    case "WAIT_PART":
                    {
                        var hasPart = dryRun || await _sensorWaiter.WaitForSensorAsync(
                            "PartPresent",
                            () => Task.FromResult(IO.ReadInput(_partPresentAddress)),
                            timeoutMs: 10000,
                            ct);
                    ...
                    case "VACUUM_ON":
                        if (!dryRun)
                        {
                            IO.WriteOutput(_vacuumOnAddress, true);
                        }
                        SetStep("WAIT_VACUUM");
                        break;
                    ...
                    case "WAIT_VACUUM":
                    {
                        ...
                        if (!vacuumOk)
                        {
                            ...
                            IO.WriteOutput(_vacuumOnAddress, false);
                            await Task.Delay(200, ct);
                            return;
                        }
    ...
    public void CaptureAutoSnapshot()
    {
        _autoSnapshot = new AutoResumeSnapshot
        {
            ...
            Y = _axisY.Position,
            Z = _axisZ.Position,
            CylinderExtend = IO.ReadOutput(_cylinderExtendAddress),
            VacuumOn = IO.ReadOutput(_vacuumOnAddress),
            Step = AutoStep
        };
    }
    ...
    public async Task<Result> RestoreAutoSnapshotAsync(CancellationToken ct = default)
    {
        ...
        IO.WriteOutput(_cylinderExtendAddress, _autoSnapshot.CylinderExtend);
        IO.WriteOutput(_vacuumOnAddress, _autoSnapshot.VacuumOn);
    ...
    private async Task<TestResult> WaitForTestResultAsync(TimeSpan timeout, bool dryRun, CancellationToken ct)
    {
        ...
        {
            if (IO.ReadInput(_testOkAddress))
            {
                return TestResult.Ok;
            }

            if (IO.ReadInput(_testNgAddress))
            {
    ...
    {
        IO.WriteOutput(_vacuumOnAddress, on);
    }

    #endregion

    public void RefreshEntityAddresses()
    {
        _partPresentAddress = ResolveAddress(PartPresentId);
        _testOkAddress = ResolveAddress(TestOkId);
        _testNgAddress = ResolveAddress(TestNgId);
        _cylinderExtendedAddress = ResolveAddress(CylinderExtendedId);
        _cylinderRetractedAddress = ResolveAddress(CylinderRetractedId);
        _cylinderExtendAddress = ResolveAddress(CylinderExtendId);
        _vacuumOkAddress = ResolveAddress(VacuumOkId);
        _vacuumOnAddress = ResolveAddress(VacuumOnId);
        _rejectCylinder = new Cylinder(IO, _cylinderExtendAddress, _cylinderExtendedAddress, _cylinderRetractedAddress, _logger);
    }

    private string ResolveAddress(string systemName)
    {
        if (_entityRegistry.TryGet(systemName, out var entity))
        {
            return entity.Address;
        }

        _logger.Warning("Entity not found for systemName {SystemName}", systemName);
        return string.Empty;
    }
}
