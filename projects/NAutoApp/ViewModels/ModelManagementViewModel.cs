...
public partial class ModelManagementViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    private NAutoAppModel? _currentModel;
    ...
    public ModelManagementViewModel(ModelService<NAutoAppModel> modelService)
    {
    ...
    private void OnModelServiceModelChanged(object? sender, ModelChangedEventArgs<NAutoAppModel> e)
    {
    ...
    private async Task CreateNewModelAsync()
    {
        try
        {
            ...
            if (CurrentModel != null)
            {
                model = (NAutoAppModel)CurrentModel.Clone(name);
                model.Description = string.Empty;
            }
            else
            {
                model = new NAutoAppModel { Name = name, CreatedAt = DateTime.Now };
            }
    ...
    private void LoadCurrentModelToEdit()
    {
        ...
        PickSpeed = CurrentModel.SpeedSettings.GetValueOrDefault(NAutoAppModel.SPEED_PICK, 20);
        PlaceSpeed = CurrentModel.SpeedSettings.GetValueOrDefault(NAutoAppModel.SPEED_PLACE, 20);

        // Delays
        VacuumOnDelay = CurrentModel.VacuumOnDelay;
        VacuumOffDelay = CurrentModel.VacuumOffDelay;
        PickDelay = CurrentModel.DelaySettings.GetValueOrDefault(NAutoAppModel.DELAY_PICK, 300);
        PlaceDelay = CurrentModel.DelaySettings.GetValueOrDefault(NAutoAppModel.DELAY_PLACE, 300);
    ...
    private void ApplyEditToModel()
    {
        ...
        // Positions
        CurrentModel.PickPosition = new TeachPosition(NAutoAppModel.POS_PICK, PickPositionX, PickPositionY, PickPositionZ);
        CurrentModel.PlacePosition = new TeachPosition(NAutoAppModel.POS_PLACE, PlacePositionX, PlacePositionY, PlacePositionZ);
        CurrentModel.HomePosition = new TeachPosition(NAutoAppModel.POS_HOME, HomePositionX, HomePositionY, HomePositionZ);

        // Speeds
        CurrentModel.MoveSpeed = MoveSpeed;
        CurrentModel.ApproachSpeed = ApproachSpeed;
        CurrentModel.SpeedSettings[NAutoAppModel.SPEED_PICK] = PickSpeed;
        CurrentModel.SpeedSettings[NAutoAppModel.SPEED_PLACE] = PlaceSpeed;

        // Delays
        CurrentModel.DelaySettings[NAutoAppModel.DELAY_VACUUM_ON] = VacuumOnDelay;
        CurrentModel.DelaySettings[NAutoAppModel.DELAY_VACUUM_OFF] = VacuumOffDelay;
        CurrentModel.DelaySettings[NAutoAppModel.DELAY_PICK] = PickDelay;
        CurrentModel.DelaySettings[NAutoAppModel.DELAY_PLACE] = PlaceDelay;
